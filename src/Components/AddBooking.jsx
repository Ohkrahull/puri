...
//   // Add a log to show the generated slots
...
//   const [filteredUsers, setFilteredUsers] = useState([]);
//   const [selectedUser, setSelectedUser] = useState(null);
//   const [selectedUserFlats, setSelectedUserFlats] = useState([]);
...
//   const [hasEndTimePermission, setHasEndTimePermission] = useState(false);

//   const startTimeDropdownRef = useRef(null);
//   const endTimeDropdownRef = useRef(null);
//   const amenityDropdownRef = useRef(null);

//   // Check if the user has admin or bookings permission
//   useEffect(() => {
//     if (user && user.roles) {
//       const hasPermission = user.roles.admin || user.roles.bookings;
//       setHasEndTimePermission(hasPermission);
//       console.log('User permissions:', { admin: user.roles.admin, bookings: user.roles.bookings });
//       console.log('Has end time permission:', hasPermission);
//     }
//   }, [user]);
...
//         return slot.timeSlotStart === time && (isEditing ? slot.id !== booking?.id : true);
//       });
...
//       // Check if it's a past time
//       if (isPastTime(editingBooking.slotDate, time)) {
//         toast.error('Cannot book past time slots');
//         return;
//       }
      
//       // Calculate end time based on amenity duration
//       const endTime = calculateEndTime(time, amenityTimeConfig.slotDuration);
//       console.log('Calculated end time:', endTime);
...
//       }
//     } else if (type === 'end') {
//       // For end time selection, ensure it's after the start time
//       const start24 = convertTo24Hour(editingBooking.slotStart);
//       const end24 = convertTo24Hour(time);
...
//       console.log('Setting end time to:', time);
//       handleInputChange('slotEnd', time);
...
//       if (isEditing && slot.id === booking.id) return false;
      
//       // Direct comparison first
//       if (slot.timeSlotStart === time) return true;
      
//       // Then try time range comparison
//       const timeToCheck = convertTo24Hour(time);
//       const startTime = convertTo24Hour(slot.timeSlotStart);
//       const endTime = convertTo24Hour(slot.timeSlotEnd);
...
//       // Set the flat ID if available
//       if (booking.flatId) {
//         setSelectedFlatId(booking.flatId);
//         // Fetch user's flats to show the selected one
//         fetchUserFlats(booking.userId);
//       }
...
//       // Always set the custom end time to current end time when editing
//       if (booking.timeSlotEnd) {
//         setCustomEndTime(booking.timeSlotEnd);
//         setUseCustomEndTime(true);
//       }
...
//               setAmenityTimeConfig(config);
              
//               // Generate all possible time slots for end time selection
//               generateAllTimeSlots(data.timeSlots.startTime, data.timeSlots.endTime);
//             }
//           }
//         }
//       };
...
//   const generateAllTimeSlots = (startTime, endTime) => {
//     console.log("Generating all time slots:", { startTime, endTime });
...
//     const slots = [];
//     let currentTime = start24;
    
//     // Generate 30-minute intervals
//     while (currentTime <= end24) {
//       slots.push(formatTimeSlot(currentTime));
//       // Add 30 minutes
//       let [hours, minutes] = currentTime.split(':').map(Number);
//       minutes += 30;
...
//     console.log("Generated all time slots:", slots);
...
//       hasEndTimePermission, 
//       roles: user?.roles,
//       isEndTimeDropdownOpen,
//       endTimeSlots: allTimeSlots.length,
//       endTimeRef: !!endTimeDropdownRef.current
//     });
    
//     // Force permission to true for development/testing if needed
//     // setHasEndTimePermission(true);
//   }, [hasEndTimePermission, isEndTimeDropdownOpen, allTimeSlots]);
...
//         // Process each user to ensure we only have one entry per user
//         users.forEach(user => {
...
//         if (userData.flats && userData.flats.approved) {
//           setSelectedUserFlats(userData.flats.approved);
//         } else {
//           setSelectedUserFlats([]);
//         }
//       }
//     } catch (error) {
...
//         date: bookingDate,
//         timeSlotStart: editingBooking.slotStart,
//         // Always use the editingBooking.slotEnd which will have been updated
//         timeSlotEnd: editingBooking.slotEnd,
//         type: 'amenities',
//         phoneNumber: selectedUser.phoneNumber || '',
//         userId: selectedUser.id,
...
//       if (editingBooking.slotDate && selectedOption) {
//         const slots = await getBookedSlots(editingBooking.slotDate, selectedOption);
//         // If we're editing, filter out the current booking's slot
//         const filteredSlots = isEditing ? 
...
//     fetchBookedSlots();
//   }, [editingBooking.slotDate, selectedOption, isEditing, booking]);
...
//     if (!selectedFlatId || !selectedUserFlats || selectedUserFlats.length === 0) {
//       return "Select Flat";
//     }
    
//     const selectedFlat = selectedUserFlats.find(flat => flat.flatId === selectedFlatId);
//     if (selectedFlat) {
...
//           {/* Flat Selection Dropdown - New */}
//           {selectedUser && selectedUserFlats && selectedUserFlats.length > 0 && (
//             <div className="mb-4" style={{ position: 'relative' }}>
//               <label style={labelStyle}>
//                 Select Flat
//               </label>
//               <div 
//                 onClick={() => setIsFlatDropdownOpen(!isFlatDropdownOpen)}
//                 style={{
//                   ...inputStyle,
//                   display: 'flex',
//                   alignItems: 'center',
...
//                     zIndex: 1001
//                   }}>
//                   {selectedUserFlats.map((flat, index) => (
//                     <div
//                       key={index}
//                       onClick={() => handleFlatSelect(flat.flatId)}
//                       style={{
//                         ...dropdownItemStyle,
//                         borderBottom: index === selectedUserFlats.length - 1 ? 'none' : '1px solid var(--Gray-100, #E5E7EB)',
//                         backgroundColor: flat.flatId === selectedFlatId ? '#F9FAFB' : 'white',
//                       }}
//                     >
//                       <div className="flex justify-between">
//                         <span>{flat.wing}-{flat.flatNumber}</span>
//                         {flat.flatId === selectedFlatId && (
//                           <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
//                             <path d="M13.3337 4L6.00033 11.3333L2.66699 8" stroke="#4F46E5" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
//                           </svg>
//                         )}
//                       </div>
//                     </div>
...
//           const isPast = isPastTime(editingBooking.slotDate, time);
//           const isBooked = isTimeSlotBooked(time);
//           const isCurrentBooking = isEditing && time === booking?.timeSlotStart;
...
//               onClick={() => {
//                 if (isSelectable) {
//                   console.log("Clicking on time:", time);
//                   handleTimeSlotSelection(time, 'start');
//                 }
//               }}
//               style={{
//                 ...dropdownItemStyle,
...
//         })
//       ) : (
//         <div style={dropdownItemStyle}>No available time slots</div>
//       )}
//     </div>
//   )}
// </div>

//                 {/* End Time - Can be edited by admins or booking managers */}
//                 <div style={{ flex: 1, position: 'relative' }}>
//   <button 
//     onClick={() => hasEndTimePermission ? setIsEndTimeDropdownOpen(!isEndTimeDropdownOpen) : null}
//     disabled={!selectedAmenityId || !editingBooking.slotStart || !hasEndTimePermission}
//     style={{
//       ...inputStyle,
//       display: 'flex',
//       alignItems: 'center',
//       justifyContent: 'space-between',
//       cursor: (selectedAmenityId && editingBooking.slotStart && hasEndTimePermission) ? 'pointer' : 'not-allowed',
//       opacity: (selectedAmenityId && editingBooking.slotStart) ? 1 : 0.7,
//       backgroundColor: hasEndTimePermission ? 'white' : '#F3F4F6',
//     }}
//   >
//     <span>{editingBooking.slotEnd || "Slot End"}</span>
//     {hasEndTimePermission && dropdownSvg}
//   </button>
//   {hasEndTimePermission && isEndTimeDropdownOpen && (
//     <div 
...
//           const startTime24 = convertTo24Hour(editingBooking.slotStart);
//           const isValid = startTime24 && endTime24 && endTime24 > startTime24;
...
//               onClick={() => {
//                 if (isValid) {
//                   console.log("Selecting end time:", time);
//                   handleTimeSlotSelection(time, 'end');
//                 }
//               }}
//               style={{
//                 ...dropdownItemStyle,
//                 borderBottom: index === allTimeSlots.length - 1 ? 'none' : '1px solid var(--Gray-100, #E5E7EB)',
//                 backgroundColor: !isValid ? '#E5E7EB' : 'white',
//                 color: !isValid ? '#9CA3AF' : 'var(--Gray-400, #6B7280)',
//                 cursor: isValid ? 'pointer' : 'not-allowed',
//               }}
//             >
//               {time} {!isValid ? '(Invalid)' : ''}
//             </div>
//           );
//         })
//       ) : (
...
//               {/* Permission message for end time */}
//               {editingBooking.slotStart && !hasEndTimePermission && (
//                 <div style={{
//                   fontSize: '12px',
//                   color: '#6B7280',
//                   marginTop: '4px',
//                   fontStyle: 'italic'
//                 }}>
//                   Note: Only admins or booking managers can modify end times
//                 </div>
//               )}
//             </div>
//           </div>
...
const AddBooking = ({ onClose, onSave, booking, isEditing = false }) => {
  ...
  useEffect(() => {
    if (isEditing && booking) {
      ...
      const fetchUserDetails = async () => {
        try {
          ...
          querySnapshot.forEach((doc) => {
            const data = doc.data();
            if (data.phoneNumber === booking.phoneNumber) {
              userInfo = {
                phoneNumber: booking.phoneNumber,
          ...
          if (userInfo) {
            ...
            setSearchTerm(`${userInfo.firstName} ${userInfo.lastName}`.trim());
          } else {
            // If no user found, just use the phone number
            setSelectedUser({ phoneNumber: booking.phoneNumber });
      ...
      if (booking.flatId) {
        setSelectedFlatId(booking.flatId);
        // Fetch user's flats to show the selected one
        fetchUserFlats(booking.phoneNumber);
      }
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        // Create a Map to store unique users by phone number
        const uniqueUsersMap = new Map();
        
        // Process each user to ensure we only have one entry per user
        users.forEach(user => {
          if (user.phoneNumber && !uniqueUsersMap.has(user.phoneNumber)) {
            uniqueUsersMap.set(user.phoneNumber, {
              phoneNumber: user.phoneNumber,
              firstName: user.firstName,
              lastName: user.lastName
            });
  ...
  const fetchUserFlats = async (phoneNumber) => {
    try {
      // Query users collection by phone number
      const usersCollectionRef = collection(db, 'users');
      const querySnapshot = await getDocs(usersCollectionRef);
      ...
      if (userData && userData.flats && userData.flats.approved) {
        setSelectedUserFlats(userData.flats.approved);
      } else {
        setSelectedUserFlats([]);
      }
  ...
  const handleUserSelect = (user) => {
    ...
    // Fetch user's flats using phone number
    fetchUserFlats(user.phoneNumber);
  };
