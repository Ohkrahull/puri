...
const NoticeTable = () => {
  ...
  useEffect(() => {
    const filtered = alerts.filter(alert => {
      const name = alert.heading?.toLowerCase() || '';
    ...
    if (typeof termOrAlert === 'object') {
      ...
      const filtered = alerts.filter(amenity => {
        return (
          ...
          amenity.category?.toLowerCase().includes(searchStr)
        );
      });
      setFilteredAlerts(filtered);
    }
  ...
  const handleSosTypeSelect = (type) => {
    ...
    if (type === "All Category") {
      ...
      const filtered = alerts.filter((alert) => alert.category === type);
      setFilteredAlerts(filtered);
    }
    setCurrentPage(1);
  };
  ...
  const handlePinClick = async (notice) => {
    try {
      // First get all pinned notices and sort by timestamp
      const pinnedNotices = alerts.filter(n => n.isPinned);
      pinnedNotices.sort((a, b) => 
        b.createdAt.toDate().getTime() - a.createdAt.toDate().getTime()
      );
  
      // Unpin all except the most recent one
      for (const pinned of pinnedNotices) {
        if (pinned.id !== notice.id && pinned.isPinned) {
          await updateDoc(doc(db, "notices", pinned.id), {
            isPinned: false
          });
        }
      }
  
      // Toggle current notice
  ...
  const handleDeleteSelected = () => {
    setDeleteItemName(`${selectedRows.length} selected notices`);
    setDeleteFunction(() => async () => {
      try {
        const noticesToDelete = selectedRows.map(index => {
          const relativeIndex = index - indexOfFirstItem;
          return currentItems[relativeIndex];
        });
  
        for (const notice of noticesToDelete) {
        ...
        toast.error("Failed to delete selected notices");
      }
    });
  ...
  const handleDeleteClick = (notice) => {
    setDeleteItemName(`Notice: ${notice.heading}`);
    setDeleteFunction(() => async () => {
      try {
        await deleteDoc(doc(db, "notices", notice.id));
              ...
              <tr className="border-y">
                {selectedRows.length > 0 ? (
                  ...
                  <>
                    ...
                    <th scope="col" className="px-2 py-3 text-left" style={{minWidth: '120px',}}></th>
                    <th scope="col" className="px-2 py-3 text-left" style={{}}></th>
                    <th scope="col" className="px-2 py-3 text-left" style={{}}></th>
                  </>
                )}
              </tr>
                ...
                currentItems.map((alert, index) => {
                  ...
                  return (
                    <tr key={alert.id} className="bg-white border-b hover:bg-gray-50" style={{fontSize:'14px',}}>
                      ...
                      <td className="px-0 py-4 whitespace-nowrap text-right" style={{minWidth: '120px'}}>
                      <div className="flex justify-end space-x-1">
                      <button onClick={() => handlePinClick(alert)} className="cursor-pointer">
  <PushPin 
    size={20} 
    weight={alert.isPinned ? "fill" : "regular"} 
    color={alert.isPinned ? "#4F46E5" : "#6B7280"}
  />
</button>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right" style={{}}>
                        <div className="flex ">
