...
const EditingDocumentModal = ({ isOpen, onClose, document, onSave }) => {
  ...
  useEffect(() => {
    if (isOpen && document) {
      setIsLoading(false);
      setEditingDocument(document);
      setDocumentType(document.documentType || 'Demand letters');
      
      // Set user search term and selected user
      const userFullName = `${document.userInfo?.firstName || ''} ${document.userInfo?.lastName || ''}`.trim();
      setSearchTerm(userFullName);
      setSelectedUser(document.userInfo || null);
      
      // Set selected flat if available
      setSelectedFlatId(document.flatId || null);
...
<div style={{...uploadAreaStyle, marginTop:'13px'}} className='flex flex-col' onClick={() => document.getElementById('fileInputEdit').click()}>
  <input
    id="fileInputEdit"
    type="file"
    onChange={handleFileChange}
...
<button 
  style={{...clearAllStyle,fontFamily:'Plus_Jakarta'}} 
  onClick={() => {
    setEditingDocument({...editingDocument, name: ''});
    setDocumentType('Demand letters');
    setFile(null);
    setSelectedUser(null);
    setSearchTerm('');
...
<button 
  ...
  style={{...saveButtonStyle, fontFamily:'Plus_Jakarta'}}
  disabled={isLoading}
>
  {isLoading ? (
