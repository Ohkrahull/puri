...
const SearchInput = ({ residents, onSearch }) => {
    ...
    const filteredResidents = residents.filter((resident) => {
      if (!searchTerm || !resident) return false;
      ...
      <div className="relative w-full sm:w-[250px] md:w-[300px]">
        <div className="flex items-center justify-between p-2 sm:py-2 border border-[#D1D5DB] rounded-[10px] text-[#6B7280] text-sm sm:text-base bg-[#F3F3F3]">
          <input
            ...
            className="w-full bg-transparent border-none outline-none shadow-none text-inherit font-inherit text-sm sm:text-base focus:ring-0 focus:outline-none"
          />
           <svg
...
const GuestLoginTable = () => {
  ...
  const handleSearch = (termOrResident) => {
    ...
    if (typeof termOrResident === 'object') {
      const name = termOrResident.firstName || termOrResident.lastName ? 
        `${termOrResident.firstName || ''} ${termOrResident.lastName || ''}`.trim() : 
        termOrResident.phoneNumber || '';
      setSearchTerm(name);
    } else {
      setSearchTerm(termOrResident.trim());
    }
  };
  ...
  const handleEditClick = (registration) => {
    Navigate(`/FlatNoForm/${registration.fullFlatNumber}`, {
      state: {
        ...
        registrationId: registration.id  // Make sure to include this
      }
    });
  };
            ...
            {currentItems.map((resident, index) => {
              ...
              return (
                <tr key={resident.id} className="bg-white border-b hover:bg-gray-50" style={{fontSize:'14px'}}>
                  ...
                  <button onClick={() => handleEditClick(resident)}>
...
export default GuestLoginTable;