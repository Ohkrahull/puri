...
const ExportModal = ({ onClose }) => {
  ...
  const { fetchConstructionUpdates } = useAuth();
  ...
  useEffect(() => {
    const loadConstructionUpdates = async () => {
      try {
        ...
        const { headers, data } = exportToCSV(fetchedUpdates);
  ...
  const exportToCSV = (updates) => {
    const headers = [
      { label: 'Update', key: 'heading' },
      { label: 'Date', key: 'date' },
      { label: 'Wings', key: 'wings' },
      { label: 'Description', key: 'subText' }
    ];
    ...
    const filteredUpdates = updates.filter(update => {
      const createdAt = update.createdAt ? update.createdAt.toDate() : null;
    ...
    const data = filteredUpdates.map(update => {
      ...
      return {
        ...
        wings: (update.selectedWings || []).join(', '),
  ...
  const generateFileName = () => {
    ...
    return `construction_updates_${formattedFromDate}_to_${formattedToDate}.csv`;
  };
