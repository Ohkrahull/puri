...
const ExportModal = ({ onClose, fetchData }) => {
  ...
  useEffect(() => {
    const loadSupportData = async () => {
      try {
        const fetchedSupport = await fetchData();
        const { headers, data } = exportToCSV(fetchedSupport);
  ...
  const exportToCSV = (supportData) => {
    ...
    const filteredSupport = supportData.filter(support => {
      const createdAt = support.createdAt ? new Date(support.createdAt.seconds * 1000) : null;
      return createdAt && createdAt >= from && createdAt <= to;
    });

    const data = filteredSupport.map(support => ({
      name: `${support.userInfo?.firstName || ''} ${support.userInfo?.lastName || ''}`.trim(),
      phoneNumber: support.phoneNumber || '',
      email: support.userInfo?.email || '',
      member: `${support.userInfo?.authorized ? 'Member' : 'Member'}`,
  ...
  const generateFileName = () => {
    ...
    return `support_export_${formattedFromDate}_to_${formattedToDate}.csv`;
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" style={{fontFamily: "Plus_Jakarta"}}>
      <div ref={modalRef} className="bg-white rounded-2xl shadow-xl w-[428px] relative" style={{fontFamily: "Plus_Jakarta"}}>
        ...
        <div className="flex justify-between items-center px-6 py-5 border-b border-gray-100">
          <div className="flex-grow text-center text-gray-900 text-lg font-semibold leading-7" style={{fontFamily: "Plus_Jakarta", fontSize:'18px'}}>Export Support</div>
