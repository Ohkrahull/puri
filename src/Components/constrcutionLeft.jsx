...
const ConstructionLeft = React.memo(({ onDataChange, data, shouldReset }) => {
  ...
  const generateUniqueId = useCallback(() => {
    return `image-${nextIdRef.current++}`;
  }, []);

  useEffect(() => {
    // Initialize images with unique ids if they don't have one
          ...
          {images.map((image, index) => (
            ...
            >
              <img 
                ...
                style={imageStyle}
                required
                onClick={() => document.getElementById(`changeImage-${image.id}`).click()}
              />
              <svg
...
export default React.memo(ConstructionLeft);