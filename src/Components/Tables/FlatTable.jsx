...
const SearchInput = ({ registrations, onSearch, authorizedUsers }) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [showDropdown, setShowDropdown] = useState(false);
...
const FlatTable = () => {
    ...
    if (registrationOrTerm && typeof registrationOrTerm === 'object') {
      ...
      const filtered = registrations.filter(reg => {
        const userMatch = reg.users?.some(user => {
          const userData = authorizedUsers[user.userId];
  ...
  const getPersonDetails = (flat, authorizedUsers) => {
    ...
    const ownerUser = ownerDetails ? authorizedUsers[ownerDetails.userId] : null;
    ...
    const tenantUser = tenantDetails ? authorizedUsers[tenantDetails.userId] : null;
            ...
            <tbody>
              {filteredRegistrations.length > 0 ? (
                currentItems.map((flat, index) => {
                  ...
                  const { ownerName, tenantName } = getPersonDetails(flat, authorizedUsers);
