...
const RentalRequestTable = () => {
  ...
  //   console.log('Query created:', rentalQuery);
    
  //   const unsubscribe = onSnapshot(rentalQuery, (snapshot) => {
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'securityDeposit':
          const depositA = parseInt(a.securityDeposit?.replace(/[^0-9]/g, '')) || 0;
          const depositB = parseInt(b.securityDeposit?.replace(/[^0-9]/g, '')) || 0;
          comparison = depositA - depositB;
          break;
        case 'availabilityDate':
          const dateA = a.availabilityDate ? a.availabilityDate.toDate().getTime() : 0;
          const dateB = b.availabilityDate ? b.availabilityDate.toDate().getTime() : 0;
  ...
  // const handleRowClick = (requestId, e) => {
  //   if (
  ...
  //     return;
  //   }
  //   navigate(`/rental-request/${requestId}`);
  // };
  ...
  <tr className="border-y">
    {selectedRows.length > 0 ? (
      ...
      <>
        ...
        {/* <th scope="col" className="px-6 py-3 text-left">Monthly Budget</th> */}
                           {/* <th scope="col" className="px-6 py-3 text-left cursor-pointer whitespace-nowrap" onClick={handleDateSort}>
                     <div className="flex items-center">
                     Monthly Budget
                    ...
                    <SortableHeader field="monthlyBudget">Monthly Budget</SortableHeader>
                    <SortableHeader field="securityDeposit">Security Deposit</SortableHeader>
                    <SortableHeader field="availabilityDate">Availability Date</SortableHeader>
        {/* <th scope="col" className="px-6 py-3 text-left">
          Security Deposit
          </th> */}
        {/* <th scope="col" className="px-6 py-3 text-left">Availability Date</th> */}
        <th scope="col" className="px-6 py-3 text-left whitespace-nowrap">Profession</th>
  ...
  {`${request.firstName || ''} ${request.lastName || ''}` || 'N/A'}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ₹{request.securityDeposit || 'N/A'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {request.availabilityDate ? dayjs(request.availabilityDate.toDate()).format('D MMM, YYYY') : 'N/A'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
