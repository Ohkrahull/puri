...
const NextSearchInput = ({ bookings, onSearch, onItemClick }) => {
    ...
    const filteredBookings = bookings && bookings.length > 0
      ? bookings.filter(
          (booking) =>
            ...
            (booking.amenity && booking.amenity.toLowerCase().includes(searchTerm.toLowerCase())) ||
            (booking.Feedback && booking.Feedback.toLowerCase().includes(searchTerm.toLowerCase()))
        )
      : [];
        ...
        {showDropdown && filteredBookings.length > 0 && (
          <div className="absolute w-full mt-3 bg-white border border-gray-200 rounded-md shadow-lg z-10">
            {filteredBookings.map((booking, index) => (
              ...
              >
                <div className="font-medium" style={{fontSize:'16px', color:'#6B7280'}}>{booking.name || 'Unknown'}</div>
              </div>
            ))}
          </div>
        )}
  ...
  export default NextSearchInput;