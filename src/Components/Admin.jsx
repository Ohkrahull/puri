...
//               {activeTab === "Amenities" ? "Bookings" : "Services"}
//             </h1>
...
//           {activeTab === "Amenities" ? (
//             <ScrollableAdminBookingTable bookings={bookings} onEditClick={handleEditClick} onDeleteClick={deleteBooking} />
//           ) : (
//             <BookingServiceTable />
//           )}
//         </div>
...
//     onSave={handleSaveEdit}
//     booking={editingBooking}
//     isEditing={true}  // New prop to indicate edit mode
//   />
//     )}
...
const Admin = () => {
  ...
  return (
    <div className="p-2 lg:p-0 lg:ml-10">
      <div style={{ position: 'relative' }}>
        <div className={`${isMobile ? 'flex-col space-y-4' : 'flex justify-between items-center'}`} style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div className={`${isMobile ? 'flex flex-row w-full gap-2' : 'flex space-x-2'} items-center justify-between`}>
            ...
            >
              <AddIcon />
              {isMobile ? 'Add Booking' : (activeTab === "Amenities" ? "Add Booking" : "Add Service")}
            </HighlightButton>
          </div>
        </div>
      </div>
      ...
      {isEditModalOpen && (
        <div className={`${isMobile ? 'fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50' : ''}`}>
          <div className={`${isMobile ? 'bg-white rounded-lg shadow-lg w-11/12 max-h-[90vh] overflow-y-auto' : ''}`}>
            <AddBooking 
              onClose={() => setIsEditModalOpen(false)}
              onSave={handleSaveEdit}
              booking={editingBooking}
