...
const GuardRegistrationForm = () => {
        ...
        try {
          ...
          if (guardDoc.exists()) {
            ...
            setFormData({
              ...
              lastName: data.lastName || '',
              email: data.email || '',
              guardPhoneNumber: data.phoneNumber || '', // Use phoneNumber from DB
              password: '', // Don't display the hashed password
              confirmPassword: '', // Don't display the hashed password
              documentType: data.documentType || 'Aadhar Card',
              imageUrl: data.imageUrl || '',
              documents: data.documents || []
            });
  ...
  const validateForm = () => {
    ...
    // Validate phone number format (allow with or without +91 prefix)
    // Will standardize when storing in the database
    const phoneRegex = /^(\+91)?[0-9]{10}$/;
    if (!phoneRegex.test(formData.guardPhoneNumber)) {
      toast.error('Please enter a valid 10-digit Indian phone number (with or without +91)');
      return false;
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      let formattedPhoneNumber = formData.guardPhoneNumber;
      if (!formattedPhoneNumber.startsWith('+91')) {
        // Remove any existing +91 just in case
        const digits = formattedPhoneNumber.replace(/\D/g, '');
        formattedPhoneNumber = `+91${digits.slice(-10)}`;
      }
      ...
      const guardData = {
        ...
        lastName: formData.lastName,
        email: formData.email,
        phoneNumber: formattedPhoneNumber, // Store formatted guard's phone number with +91
        imageUrl: formData.imageUrl,
        documentType: formData.documentType,
        documents: formData.documents,
        createdAt: new Date(),
      ...
      if (formData.password) {
        guardData.password = await hashPassword(formData.password);
      }
      ...
      if (id) {
        ...
        if (!formData.password && guardDoc.exists()) {
          // Keep existing password if no new one is provided
          guardData.password = guardDoc.data().password;
        }
        
        await updateDoc(guardRef, guardData);
        toast.success('Guard updated successfully');
      } else {
        // Create new guard
        const employeeId = await generateUniqueEmployeeId(db);
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 font-medium">
                    {!formData.guardPhoneNumber.startsWith('+91') && formData.guardPhoneNumber.length > 0 ? '+91' : ''}
                  </span>
                  <input
                    type="tel"
                    name="guardPhoneNumber"
                    ...
                    className={`w-full px-4 ${!formData.guardPhoneNumber.startsWith('+91') && formData.guardPhoneNumber.length > 0 ? 'pl-12' : ''} h-[45px] bg-[#F9FAFB] border border-[#E5E7EB] rounded-lg text-[#4B5563] text-base outline-none`}
                    style={{fontSize: 16}}
                  />
                </div>
