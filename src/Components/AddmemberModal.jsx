...
import { addNewAuthorizedUser } from '../firebase/services/UserData';
import { checkIfEmailExists, checkIfPhoneExists } from '../firebase/services/UserService';
...
const AddMemberModal = ({ isOpen, onClose, onAdd }) => {
  ...
  //     setMember(prev => ({...prev, [name]: '+91' + numericValue}));
  //   } else if (name === 'flatNumber') {
  //     const numericValue = value.replace(/\D/g, '').slice(0, 5);
  //     setMember(prev => ({...prev, [name]: numericValue}));
  //   } else {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'phone') {
      ...
      // Store the value without the +91 prefix
  ...
  const handleWingSelect = (wing) => {
    setMember(prev => ({ ...prev, wing }));
    setIsWingDropdownOpen(false);
  };

  const handleAdd = async () => {
    if (!member.firstName || !member.lastName || !member.email || !member.phone || member.wing === 'Choose Wing' || !member.flatNumber) {
      toast.error('Please fill in all fields');
      return;
    }

    try {
      const emailExists = await checkIfEmailExists(member.email);
      if (emailExists) {
        toast.error("This email is already registered. Please use a different email address.");
        return;
      }

      const phoneExists = await checkIfPhoneExists(member.phone);
      if (phoneExists) {
        toast.error("This phone number is already registered. Please use a different phone number.");
        return;
      }

      await addNewAuthorizedUser(member.phone, member.firstName, member.lastName, member.email, member.wing, member.flatNumber);
      console.log("New Member added successfully: ", member);
      onAdd(member);
      resetForm();
      onClose();
    } catch (error) {
  ...
  const isFormValid = () => {
    return member.firstName && member.lastName && member.email && member.phone && member.wing !== 'Choose Wing' && member.flatNumber;
  };
    ...
    <div style={{
      ...
      width: '428px',
          ...
          <div style={{
            ...
            <input
              ...
              style={{
                display: 'flex',
                height: '44px',
                padding: '10px 14px 10px 44px', // Add left padding to accommodate +91
                alignItems: 'center',
                gap: '8px',
                alignSelf: 'stretch',
                borderRadius: '8px',
          ...
          <div style={{ width: '100%', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              <div style={{ flex: 1, position: 'relative' }}>
                ...
                >
                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{member.wing}</span>
        ...
        <div style={{
          ...
          <button onClick={handleAdd} disabled={!isFormValid()} style={{
            display: 'flex',
            padding: '10px 20px',
