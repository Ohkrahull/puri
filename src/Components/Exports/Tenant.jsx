...
const TenantExportModal = ({ onClose }) => {
  ...
  const getHeaders = () => {
    return [
      ...
      { label: 'Rental Budget', key: 'monthlyBudget' },
      { label: 'Lease Tenure', key: 'leaseTenure' },
  ...
  const handleExport = () => {
    ...
    const formattedData = filteredData.map(request => ({
      ...
      leaseTenure: request.leaseTenure ? `${Math.round(request.leaseTenure)} Months` : 'N/A',
      moveInDate: request.moveInDate 
        ? dayjs(request.moveInDate.toDate()).format('D MMM, YYYY') 
        : 'N/A',
      furnishingType: request.furnishingType || 'N/A',
      bhkType: request.bhkType || 'N/A',
    ...
    if (formattedData.length === 0) {
      toast.info("No tenant requests found in the selected date range");
    }
  ...
  const generateFileName = () => {
    ...
    return `tenant_requests_${formattedFromDate}_to_${formattedToDate}.csv`;
  };
...
export default TenantExportModal;