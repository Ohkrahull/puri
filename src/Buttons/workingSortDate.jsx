...
const SortButton = ({ onSort }) => {
  ...
  const handleDateSelection = (newValue) => {
    ...
    if (newValue.startDate && newValue.endDate) {
      ...
      onSort(startDate.toDate(), endDate.toDate());
      setIsCalendarOpen(false);
    }
  };
  ...
  const filterBookings = (option) => {
    ...
    if (startDate && endDate) {
      setSelectedOption(`${startDate.format('DD MMM, YYYY')} - ${endDate.format('DD MMM, YYYY')}`);
    } else {
      setSelectedOption(option);
    }

    onSort(startDate?.toDate() || null, endDate?.toDate() || null);
  };
