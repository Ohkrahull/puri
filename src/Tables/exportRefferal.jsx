...
const ExportModal = ({ onClose }) => {
  ...
  useEffect(() => {
    const loadReferrals = async () => {
      try {
        const fetchedReferrals = await fetchReferrals();
        const { headers, data } = exportToCSV(fetchedReferrals);
  ...
  const exportToCSV = (referrals) => {
    const headers = [
      { label: 'First Name', key: 'firstName' },
      { label: 'Last Name', key: 'lastName' },
      { label: 'Email', key: 'email' },
      { label: 'Phone Number', key: 'phoneNumber' },
      { label: 'Referred Name', key: 'referredName' },
      { label: 'Referred Email', key: 'referredEmail' },
      { label: 'Referred Phone Number', key: 'referredPhoneNumber' },
      { label: 'Relationship', key: 'relationship' },
      { label: 'Contact Method', key: 'contactMethod' },
      { label: 'Interests', key: 'interests' },
      { label: 'Is Guest', key: 'isGuest' },
    ...
    const filteredReferrals = referrals.filter(referral => {
      const createdAt = referral.createdAt ? new Date(referral.createdAt.seconds * 1000) : null;
      return createdAt && createdAt >= from && createdAt <= to;
    });

    const data = filteredReferrals.map(referral => ({
      firstName: referral.userInfo?.firstName || '',
      lastName: referral.userInfo?.lastName || '',
      email: referral.userInfo?.email || '',
      phoneNumber: referral.userInfo?.phoneNumber || referral.phoneNumber || '',
      referredName: referral.referredName || '',
      referredEmail: referral.referredEmail || '',
      referredPhoneNumber: referral.referredPhoneNumber || '',
      relationship: referral.relationship || '',
      contactMethod: referral.contactMethod || '',
      interests: Array.isArray(referral.interests) ? referral.interests.join(', ') : '',
      isGuest: referral.isGuest ? 'Yes' : 'No',
  ...
  const generateFileName = () => {
    ...
    return `referrals_export_${formattedFromDate}_to_${formattedToDate}.csv`;
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" style={{fontFamily: "Plus_Jakarta"}}>
      <div ref={modalRef} className="bg-white rounded-2xl shadow-xl w-[428px] relative" style={{fontFamily: "Plus_Jakarta"}}>
        ...
        <div className="flex justify-between items-center px-6 py-5 border-b border-gray-100">
          <div className="flex-grow text-center text-gray-900 text-lg font-semibold leading-7" style={{fontFamily: "Plus_Jakarta", fontSize:'18px'}}>Export Referrals</div>
