...
const SearchInput = ({ documents, onSearch }) => {
  ...
  //   onSearch(`${document.userInfo?.firstName || ''} ${document.userInfo?.lastName || ''}`);
  // };
  // const handleItemClick = (document) => {
  //   setSearchTerm(`${document.userInfo?.firstName || ''} ${document.userInfo?.lastName || ''}`);
  //   setShowDropdown(false);
...
const DemandLetter = () => {
  ...
  useEffect(() => {
    const fetchDocuments = () => {
      ...
      const q = query(
        ...
        where('documentType', '==', 'Demand letters')
      );
  ...
  // const filterDocuments = (termOrObject, status, docsToFilter = sortedDocuments) => {
  //   let filtered = docsToFilter;
    
  //   if (termOrObject) {
  //     if (typeof termOrObject === 'string') {
  //       const term = termOrObject.toLowerCase();
  //       filtered = filtered.filter(doc =>
  //         (doc.userInfo?.firstName && doc.userInfo.firstName.toLowerCase().includes(term)) ||
  //         (doc.userInfo?.lastName && doc.userInfo.lastName.toLowerCase().includes(term)) ||
  //         doc.name.toLowerCase().includes(term) ||
  //         doc.fileName.toLowerCase().includes(term)
  //       );
  //     } else {
  //       // If an object is passed, filter for exact match
  //       filtered = filtered.filter(doc =>
  //         doc.id === termOrObject.id ||
  //         (doc.userInfo?.firstName === termOrObject.userInfo?.firstName &&
  //          doc.userInfo?.lastName === termOrObject.userInfo?.lastName)
  //       );
  //     }
  //   }
  ...
  //   setSearchTerm(term);
  //   filterDocuments(term, selectedStatus, sortedDocuments);
  // };
  // const handleSearch = (termOrObject) => {
  //   if (typeof termOrObject === 'string') {
  //     setSearchTerm(termOrObject);
  //     setSearchObject(null);
  //   } else {
  //     setSearchTerm(`${termOrObject.userInfo?.firstName || ''} ${termOrObject.userInfo?.lastName || ''}`);
  //     setSearchObject(termOrObject);
  //   }
  //   filterDocuments(termOrObject, selectedStatus, sortedDocuments);
  // };
  ...
  const renderTableContent = () => {
    ...
    return currentItems.map((doc, index) => {
      ...
      const displayName = doc.userInfo?.firstName && doc.userInfo?.lastName 
        ? `${doc.userInfo.firstName} ${doc.userInfo.lastName}`
        : doc.name || 'N/A';
      ...
      return (
        <tr key={doc.id} className="bg-white border-b hover:bg-gray-50" style={{fontSize:'14px'}}>
          ...
          <td className="px-8 py-4 text-left" style={{width: '30%'}}>{displayName}</td>
          ...
          <span
            ...
            >
              <div className="flex text-center justify-center items-center">
                ...
                ></span>
                <span style={{ fontSize: '12px' }}>{doc.status}</span>
              </div>
            </span>
          </td>
          {/* <td className="px-6 py-4 text-center" style={{width: '20%'}}>{uploadTime}</td> */}
          {/* <td className="px-6 py-4 text-center" style={{width: '20%'}}>{uploadDate}</td> */}
  ...
  return (
    <div className=" bg-white   overflow-hidden flex flex-col" style={{ height: '320px' }}>
          ...
          <table className="w-full text-sm text-gray-500">
            <thead className="text-xs text-gray-500 sticky top-0 z-10 bg-gray-50" style={{fontSize:'12px'}}>
              <tr className="border-y">
                {/* <th scope="col" className="p-4 w-4"style={{width:'5%'}}>
                  
                </th> */}
                <th scope="col" className="py-4 px-8 text-left " style={{fontSize:'12px', width: '30%'}}>Name</th>
                <th scope="col" className="px-6 py-4 text-left" style={{width: '30%'}}>Status</th>
      ...
      <div className="flex-grow overflow-y-auto" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', '&::-webkit-scrollbar': { display: 'none' } }}>
